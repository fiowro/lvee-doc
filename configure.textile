...
After the "installation":http://lvee.org/uk/wiki_pages/[org]%20LVEE%20engine%20-%20installation lvee engine still needs to surpass some configuration. 
...
User should be created via website, by clicking the "register" link on top. After that you should run rails console in the terminal and set "role" attribute equal to "admin" for the first user, and, perhaps, write down some date of the account activation (to be able to login with new user):
<CODE>
source /home/vasya/.rvm/scripts/rvm
cd ./lvee
rails c
User.first.update_attribute("role", "admin")
User.first.update_attribute("activated_at", "2001-02-03 01:02:03")
quit
</CODE>
...
Do not forget to use your real gmail account name and password.After the "installation":http://lvee.org/uk/wiki_pages/[org]%20LVEE%20engine%20-%20installation lvee engine still needs to surpass some configuration. 
